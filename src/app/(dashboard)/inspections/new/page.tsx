...
export default function NewInspectionPage() {
  ...
  const createInspection = useMutation({
    mutationFn: async () => {
      // Upload images to S3 with optimization
      const imageUrls: string[] = []
      
      for (const file of images) {
        // Use optimized upload endpoint that resizes images before storing
        const { imageUrl } = await apiClient.uploadInspectionImage(file)
        imageUrls.push(imageUrl)
      }

      // Create inspection with S3 URLs (inspectionId and link will be auto-generated by backend)
